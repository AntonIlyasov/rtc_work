...
#define DATA_FROM_UDP_SIZE                     50 // [AA][BB][LEN][6f*6b][HM][HPos][CAM][UR][REL][CAM_GPIO][4b KeepAL][CRC]
#define DATA_TO_UDP_SIZE                       77 // [BB][AA][LEN][6f*10b][HM][HPos][CAM][24v][48v][Ok][UR][RL][CAM_GPIO][4b KeepAL][CRC]
...
#define DATA_FROM_UDP_SIZE_SETTINGS            68 // [AA][BB][LEN][6f*9b][HM][HPos][CAM][UR][REL][CAM_GPIO][4b KeepAL][CRC]
#define DATA_TO_UDP_SIZE_SETTINGS              71 // [BB][AA][LEN][6f*9b][HM][HPos][CAM][24v][48v][Ok][UR][RL][CAM_GPIO][4b KeepAL][CRC]
...
#define TO_CAM_GPIO_TOPIC_NAME      "toCamGPIOTopic"
#define FROM_CAM_GPIO_TOPIC_NAME    "fromCamGPIOTopic"
...
/* 
  ...
  Класс UDPServer включает следующие методы:
    ...
    * nodeFromUDPProcess()  - основной цикл программы, который состоит из последовательности действий:
        1) отправка в ROS-топик данных на реле камеры GPIO                  - метод sendMsgToCamGPIO()
        2) отправка в ROS-топик данных на Кисть                             - метод sendMsgToFingers()
        3) отправка в ROS-топик данных на Плату Управления (АКБ, камеры)    - метод sendMsgToBatCam()
        4) отправка по протоколу UDP данных пользователю                    - метод sendMsgToUDP()
    * from_cam_gpio_handle_receive()  - обработчик сообщений от ROS-топика с реле камеры GPIO 
    ...
    * sendMsgKeepAlive()              - отправка пакета в топик "toKeepAliveTopic" для отслеживания принимаемых по UDP пакетов
*/
...
public:
  ...
  // конструктор класса UDPServer, в котором происходит подключение к сети по протоколу UDP, и подключение к ROS-топикам 
    ...
    toCamGPIOPub    = node.advertise<std_msgs::UInt8>(TO_CAM_GPIO_TOPIC_NAME,           0);
    fromCamGPIOSub  = node.subscribe<std_msgs::UInt8>(FROM_CAM_GPIO_TOPIC_NAME,         0,  
        &UDPServer::from_cam_gpio_handle_receive, this);
...
private:
  ...
  ros::Publisher  toCamGPIOPub;
  ros::Subscriber fromCamGPIOSub;
  ...
  // обработчик сообщений от ROS-топика с реле камеры GPIO
  void from_cam_gpio_handle_receive(const std_msgs::UInt8::ConstPtr& recvdMsg){
    currentState.cam_status_from_gpio = recvdMsg->data;
  }
  ...
  */
  ...
  /*
    отправка пакета в топик "toKeepAliveTopic" для отслеживания принимаемых по UDP пакетов
  */
  void sendMsgKeepAlive(){
    std_msgs::UInt32 sendMsgToKeepAliveTopic;
    uint32_t keepAlive32 = (currentState.keepalive[3] << 24) + (currentState.keepalive[2] << 16) + 
                           (currentState.keepalive[1] << 8)  +  currentState.keepalive[0];
  ...
  */
  void udp_handle_receive(const boost::system::error_code& error, uint32_t bytes_transferred) {
    ...
    getMsgFromUDP                     = true;
    currentState.hold_position        = dataFromUDP[dataFromUDP[2] - 10 * sizeof(uint8_t)];
    currentState.camera_from_udp      = dataFromUDP[dataFromUDP[2] - 9  * sizeof(uint8_t)];
    currentState.ur5_from_udp         = dataFromUDP[dataFromUDP[2] - 8  * sizeof(uint8_t)];
    currentState.relay_from_udp       = dataFromUDP[dataFromUDP[2] - 7  * sizeof(uint8_t)];
    currentState.cam_gpio_from_udp    = dataFromUDP[dataFromUDP[2] - 6  * sizeof(uint8_t)];
    memcpy(currentState.keepalive,    dataFromUDP + dataFromUDP[2] - 5  * sizeof(uint8_t), sizeof(currentState.keepalive));
    if (settings) {
      std::cout << "\033[1;31mCALIBRATE DATA FROM UDP\033[0m\n";
  ...
  // отправка в ROS-топик данных на реле камеры GPIO
  void sendMsgToCamGPIO(){
    std_msgs::UInt8 sendMsgToCamGPIOTopic;
    sendMsgToCamGPIOTopic.data = currentState.cam_gpio_from_udp;
    toCamGPIOPub.publish(sendMsgToCamGPIOTopic);
    std::cout << "\033[1;34mSEND TO toCamGPIOTopic:\033[0m\n";
    printf("cam_gpio_from_udp = [%u]\n",  currentState.cam_gpio_from_udp);
  ...
  void sendMsgToUDP(){
    ...
    if (resvdBytesFromFingers == DATA_FROM_FINGERS_TOPIC_SIZE){
      ...
      dataToUDP[sizeof(dataToUDP) - 14 * sizeof(uint8_t)] = currentState.hand_mount;                          //hand_mount               1b
      dataToUDP[sizeof(dataToUDP) - 13 * sizeof(uint8_t)] = currentState.hold_position;                       //hold_position            1b
      dataToUDP[sizeof(dataToUDP) - 12 * sizeof(uint8_t)] = currentState.camera_from_bat_cam;                 //camera_from_bat_cam      1b
      dataToUDP[sizeof(dataToUDP) - 11 * sizeof(uint8_t)] = currentState.bat_24V;                             //bat_24V                  1b
      dataToUDP[sizeof(dataToUDP) - 10  * sizeof(uint8_t)] = currentState.bat_48V;                            //bat_48V                  1b
      dataToUDP[sizeof(dataToUDP) - 9  * sizeof(uint8_t)] = allDevOk;                                         //allDevOk                 1b
      dataToUDP[sizeof(dataToUDP) - 8  * sizeof(uint8_t)] = currentState.ur5_from_bat_cam;                    //ur5_state                1b
      dataToUDP[sizeof(dataToUDP) - 7  * sizeof(uint8_t)] = currentState.relay_from_bat_cam;                  //relay_state              1b
      dataToUDP[sizeof(dataToUDP) - 6  * sizeof(uint8_t)] = currentState.cam_status_from_gpio;                //cam_gpio                 1b
      ...
      dataToUDP_setting[sizeof(dataToUDP_setting) - 14 * sizeof(uint8_t)] = currentState.hand_mount;          //hand_mount              1b
      dataToUDP_setting[sizeof(dataToUDP_setting) - 13 * sizeof(uint8_t)] = currentState.hold_position;       //hold_position           1b
      dataToUDP_setting[sizeof(dataToUDP_setting) - 12 * sizeof(uint8_t)] = currentState.camera_from_bat_cam; //camera_from_bat_cam     1b
      dataToUDP_setting[sizeof(dataToUDP_setting) - 11 * sizeof(uint8_t)] = currentState.bat_24V;             //bat_24V                 1b
      dataToUDP_setting[sizeof(dataToUDP_setting) - 10 * sizeof(uint8_t)] = currentState.bat_48V;             //bat_48V                 1b
      dataToUDP_setting[sizeof(dataToUDP_setting) - 9  * sizeof(uint8_t)] = allDevOk;                         //allDevOk                1b
      dataToUDP_setting[sizeof(dataToUDP_setting) - 8  * sizeof(uint8_t)] = currentState.ur5_from_bat_cam;    //ur5_state               1b
      dataToUDP_setting[sizeof(dataToUDP_setting) - 7  * sizeof(uint8_t)] = currentState.relay_from_bat_cam;  //relay_state             1b
      dataToUDP_setting[sizeof(dataToUDP_setting) - 6  * sizeof(uint8_t)] = currentState.cam_status_from_gpio;//cam_gpio                1b
